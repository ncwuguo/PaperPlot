...
T, R, B0, B1, C, A = np.meshgrid(t, r, b0, b1, c, a)
...
gap = np.where(condition, equation, np.nan)

# The impact of b1 on the equilibrium quantity gap: dict format
b1_abs_dict = {
    b1_val: np.where(
        gap[B1 == b1_val] > 0, 1, np.where(gap[B1 == b1_val] < 0, 0, np.nan)
    )
    for b1_val in b1
}
...
for i, ax in enumerate(axes):
    ax.set_xlabel(props_xlabels[i], fontproperties=props_font_label)
